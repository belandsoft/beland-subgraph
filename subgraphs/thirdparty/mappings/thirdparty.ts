...
import { Curation, Receipt, ThirdParty } from "../generated/schema";
import { isURNValid } from "../utils/thirdparty";
import { buildMetadata } from "../utils/metadata";
import { setThirdPartySearchFields } from "../utils/metadata/thirdparty";
import { buildCountFromCuration, buildCountFromReceipt, buildCountFromThirdParty } from "../utils/count";
import {
  ItemSlotsConsumed,
  ThirdPartyAdded,
  ...
  ThirdPartyUpdated,
} from "../generated/ThirdPartyRegistry/ThirdPartyRegistry";

export function handleThirdPartyAdded(event: ThirdPartyAdded): void {
  if (!isURNValid(event.params._thirdPartyId)) {
    log.error('A third party was added with an invalid URN as an id "{}"', [event.params._thirdPartyId]);
    return;
  }
...
export function handleThirdPartyItemSlotsBought(event: ThirdPartyItemSlotsBought): void {
  let thirdParty = ThirdParty.load(event.params._thirdPartyId);

  if (thirdParty == null) {
    log.error('A non existent Third Party with id "{}" bought "{}" item slots', [
...
export function handleThirdPartyReviewedWithRoot(event: ThirdPartyReviewedWithRoot): void {
  ...
  if (thirdParty == null) {
    log.error("Attempted to review with root using an unregistered third party with id {}", [thirdPartyId]);
    return;
  }
...
export function handleThirdPartyReviewed(event: ThirdPartyReviewed): void {
  ...
  if (thirdParty == null) {
    log.error("Attempted to review an unregistered third party with id {}", [thirdPartyId]);
    return;
  }
...
export function handleItemSlotsConsumed(event: ItemSlotsConsumed): void {
  ...
  if (thirdParty == null) {
    log.error("Tried to consume slots for unregistered third party with id {}", [thirdPartyId]);
    return;
  }
